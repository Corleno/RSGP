...
# compute the logN(Y|0, S = K_12K_22^-1K_21 + D) d is a vector.
def log_Sparse_Guassian(Y, K12, K22, K21, d):
    ...
    cc = y.dot(bb)
    dd = K21.dot(bb)
    ee = np.linalg.solve(A, dd)
    ff = K12.dot(ee)
    gg = ff/d
    s = cc - y.dot(gg)
    logdeterminantS = np.linalg.slogdet(A)[1] - np.linalg.slogdet(K22)[1] + np.sum(np.log(d))
    res = -0.5*N*np.log(2*np.pi) - 0.5*logdeterminantS - 0.5*s
    # import pdb
    # pdb.set_trace()
    return res


def log_Sparse_Guassian_validation(Y, K12, K22, K21, d):
    Q = K12.dot(np.linalg.inv(K22)).dot(K21) 
    return multivariate_normal.logpdf(Y.reshape(-1), mean=np.zeros_like(Y.reshape(-1)), cov=Q+np.diag(d))
...
def log_likelihood(hyper_parameters, X, Y, Z, kernel=RBF_cov):
    ...
    d = sigma2_err + sigma2 - np.diag(K12.dot(np.linalg.inv(K22)).dot(K21)) 
    llk = log_Sparse_Guassian(Y, K12, K22, K21, d)
    # vllk = log_Sparse_Guassian_validation(Y, K12, K22, K21, d)
...
def prediction(X, Y, Z, X_pred, hyper_parameters, kernel=RBF_cov):
    ...
    B = sigma2_err**(-1)* np.eye(N) - sigma2_err**(-2)*(K12.dot(np.linalg.inv(A)).dot(K21)) 
    mu_pred = kernel(X_pred, Z, sigma2, l).dot(np.linalg.solve(K22, (K21.dot(B.dot(Y)))))
    return mu_pred
