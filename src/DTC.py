...
def training_hyper(X, Y, Z, hyper_parameters=np.zeros(3), kernel=RBF_cov):
    ...
    print(objective_function_hyper(hyper_parameters, X, Y, Z))
...
def prediction(X, Y, Z, X_pred, hyper_parameters, kernel=RBF_cov):
    ...
    mu_pred = kernel(X_pred, Z, sigma2, l).dot(np.linalg.inv(K22).dot(K21.dot(B.dot(Y))))
    return mu_pred
...
if __name__ == "__main__":
    ...
    # ### Select optimal lambda using holdout dataset
